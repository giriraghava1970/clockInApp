...
import { Card, Button, Container, Footer, FooterTab, Icon } from 'native-base';
...
export default class PageOne extends Component {
    ...
    constructor(props) {
        ...
        console.log('total',this.state.totalLength)
    ...
    componentDidMount = async() => {
        ...
        this.watchID = Geolocation.watchPosition((position) => 
            ...
            (error) => alert(JSON.stringify(error)),
            {enableHighAccuracy: true, timeout: 20000, maximumAge: 1000, distanceFilter:0.01, interval:1000}
        );
    }
    ...
    startShift = async() =>{
        ...
        if(responseJson.rows[0].elements[0].status=='OK'){
            if(Number(responseJson.rows[0].elements[0].distance.value) <= 15000){
                this.setState({
                    startButton: false
               })
            }
